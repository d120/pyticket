...
class Rule(object):
    ...
    def __init__(
        self, freq,
        interval=1, wkst=None, count=None, until=None, dtstart=None, **kwargs
    ):
        """
        Create a new rule.
    ...
    def __hash__(self):
        ...
        return hash((
            self.freq, self.interval, self.wkst, self.count, self.until, self.dtstart,
    ...
    def to_dateutil_rrule(self, dtstart=None, dtend=None, cache=False):
        ...
        dtstart = self.dtstart or dtstart
        until = self.until
    ...
    def validate_rule(rule):
        ...
        if rule.dtstart:
            try:
                ...
                raise exceptions.ValidationError(
                    'invalid dtstart parameter: %r' % rule.dtstart)
...
def serialize(rule_or_recurrence):
    ...
    for rrule in obj.rrules:
        if int(rrule.interval) > 0:
            if hasattr(rrule,'dtstart') and rrule.dtstart is None:
                dt = datetime.datetime.now().replace(hour=12)
                rrule.dtstart = localtz().localize(dt).astimezone(pytz.utc)
...
def from_dateutil_rrule(rrule):
    ...
    elif rrule.dtstart is not None:
